...
class NetworkRequestStorageManagerTest {
    ...
    fun `getAllStoredRequests returns combined requests from all interceptors sorted by timestamp`() {
        ...
        val request1 = StoredNetworkRequest("url1", "GET", emptyMap(), null, 1000L, 200, "response1")
        val request2 = StoredNetworkRequest("url2", "POST", emptyMap(), "body", 2000L, 201, "response2")
    ...
    fun `clearAllStoredRequests clears all interceptors`() {
        ...
        interceptor1.addRequest(StoredNetworkRequest("url1", "GET", emptyMap(), null, 1000L, 200, "response1"))
        interceptor2.addRequest(StoredNetworkRequest("url2", "POST", emptyMap(), "body", 2000L, 201, "response2"))
