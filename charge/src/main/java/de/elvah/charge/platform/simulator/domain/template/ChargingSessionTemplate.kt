...
internal abstract class ChargingSessionTemplate(
     ...
     */
    final fun processSession(context: SimulationContext): ChargingSession? {
     ...
     */
    protected fun isInitialState(context: SimulationContext): Boolean = context.currentSession == null
    
    /**
     * Checks if we're in the starting phase.
     */
    protected fun isStartingPhase(context: SimulationContext): Boolean {
        return context.currentStatus in listOf(SessionStatus.START_REQUESTED, SessionStatus.START_REJECTED)
    }
     ...
     */
    protected open fun postProcessSession(session: ChargingSession?, context: SimulationContext): ChargingSession? {
        return session
    }
